...
def get_menu_choice():
    ...
    print()
    pick = input('Enter your choice: ') #set choice to what user chooses
    while (len(pick) != 1) or (ord(pick) not in range(48,55)): #if invalid keep asking for input until valid (less than 6) entered
        pick = (input('Enter a valid choice: '))
        
    return int(pick) #once loop is satisfied (meaning a valid num is entered) choice is returned
